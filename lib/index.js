  ...
  function write(out, astNode) {
    if (astNode.type === NODES.ROOT) {
      ...
      if (firstSym === 'def') {
        ...
        assert(astNode, nodes[2].type === NODES.LIST, '"fn" needs an argument list, got atom of type: ' + nodes[2].type);
  ...
  function ePrint(ast) {
    ...
    function astToString(node) {
      if ('nodes' in node) {
        ...
        return '"' + replaceEscapes(astNode.value) + '"';
      } else {
        return node.value;
      }
    }
