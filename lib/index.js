(function(GLOBAL) {
  ...
  function write(out, astNode) {
    if (astNode.type === NODES.ROOT) {
      ...
      // (fn (...) ...) -> function(...) { return ...; }
      if (astNode.nodes.length > 0 && astNode.nodes[0].value === 'fn') {
        __eth__assert(astNode.nodes.length >= 3, '"fn" needs arguments and a body');
        __eth__assert(astNode.nodes[1].type === NODES.LIST, '"fn" needs an argument list, got atom of type: ' + astNode.nodes[1].type);
        __eth__assert(astNode.nodes[1].nodes.reduce(function(isAllSymbol, node) {
          return isAllSymbol && node.type === NODES.SYMBOL;
        }, true), '"fn" needs arguments list to only contain symbols');
        return [
          out,
          '(function (',
          writeCommaSeparatedValues(astNode.nodes[1].nodes),
          ') { return ',
          write('', newAstList([newAstSym('do')].concat(astNode.nodes.slice(2)))),
          '; })'
        ].join('');
      }

      // (do ...) -> (function() { ... return ...; })()
      if (astNode.nodes.length > 1 && astNode.nodes[0].value === 'do') {
        bodyNodes = astNode.nodes.slice(1, -1).map(function(node) {
          return write('', node);
        });
        return [
          out,
          ...
          bodyNodes.join(';\n'),
          'return ',
          write('', astNode.nodes[astNode.nodes.length-1]),
          ';\n})()'
        ].join('');
      }

      // (add 1 2) => add(1, 2);
      var nodesOut = astNode.nodes.slice(1).map(function(node, i) {
