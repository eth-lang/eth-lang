(function(GLOBAL) {
  ...
  function write(out, astNode) {
    if (astNode.type === NODES.ROOT) {
      ...
      if (astNode.value.length > 0 && astNode.value === '@') {
        return out + 'this';
      }
      if (astNode.value.length > 0 && astNode.value[0] === '@') {
        return out + 'this.' + astNode.value.slice(1);
      }
      if (astNode.value.length > 0 && astNode.value.endsWith('!')) {
        return out + 'this.' + astNode.value.slice(0, -1) + '$';
      }
      return out + astNode.value.replace(/-(.)/g, function(_, groupOne) {
